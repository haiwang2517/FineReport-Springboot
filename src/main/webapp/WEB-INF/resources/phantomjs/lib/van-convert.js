...
//phantomjs离开时输出信息
var statusMsg, chartType, pageMsg, renderMsg , responseMsg;
...
function render(arg, cb) {
    ...
    page.clipRect = { top: 0, left: 0, width: (arg.width || DEFAULT_WIDTH), height: (arg.height || DEFAULT_HEIGHT)};
    ...
    server.listen(host ? host + ':' + port : parseInt(port, 10),
        function (request, response) {
            ...
            if ('status' in params) {
                ...
                chartType = params.option.chartType;
...
function exit (code) {
    ...
    clog.warn('chartType:' + chartType);
    clog.warn('pageMsg:' + pageMsg);
    clog.warn('renderMsg:' + renderMsg);
    clog.warn('statusMsg:' + statusMsg);
    clog.warn('responseMsg:' + responseMsg);
